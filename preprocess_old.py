...
def get_mosei():
    ...
    alldata=pickle.load(open("/home/jiawei/COGMEN/emotion_recognition/data/mosei/mosei_prepare_data.pkl","rb"))
    train_labels=alldata["train"]["labels"]
    train_text=alldata["train"]["text"]
    train_audio=alldata["train"]["audio"]
    train_visual=alldata["train"]["visual"]
    ...
    for dialogue_idx in tqdm(train_vids,desc="train"):
        ...
        if args.dataset == "mosei":
            labels = [
                cmumosei_7(train_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        elif args.dataset == "mosei_2":
            labels = [
                cmumosei_2(train_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        train.append(
            cogmen.Sample(
                str(dialogue_idx), speakers, labels, text, audio, visual, text,dataset=args.dataset
            )
        )
    ...
    for dialogue_idx in tqdm(valid_vids,desc="dev"):
        ...
        if args.dataset == "mosei":
            labels = [
                cmumosei_7(valid_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        elif args.dataset == "mosei_2":
            labels = [
                cmumosei_2(valid_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        dev.append(
            cogmen.Sample(
    ...
    for dialogue_idx in tqdm(test_vids,desc="test"):
        ...
        if args.dataset == "mosei":
            labels = [
                cmumosei_7(test_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        elif args.dataset == "mosei_2":
            labels = [
                cmumosei_2(test_labels[dialogue_idx][i][0])
                for i in range(num_of_utterances)
            ]
        test.append(
            cogmen.Sample(
